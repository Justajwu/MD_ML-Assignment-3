...
```{r}
#run ravi's code importing 2008-2016
getwd()
...
sqf.data <- sqf.data %>% mutate(suspect.height = standardize(suspect.height),
...
yvar <- 'found.weapon'
xvars <- c('precinct','location.housing', 'additional.proximity' , 'additional.highcrime', 'additional.evasive' , 'additional.time', 'additional.associating', 'additional.report', 'additional.sights', 'additional.other', 'additional.investigation', 'additional.direction', 'stopped.bc.desc', 'stopped.bc.other', 'stopped.bc.casing', 'stopped.bc.furtive', 'stopped.bc.lookout', 'stopped.bc.violent', 'stopped.bc.clothing', 'stopped.bc.bulge', 'age', 'suspect.build', 'suspect.sex', 'suspect.height', 'suspect.weight', 'radio.run', 'inside', 'observation.period', 'day', 'month', 'time.period')


#Part i model
fit <- glm( found.weapon ~ ., data=sqf.data[, c(yvar,xvars)] )

#coefficients
exp(coefficients(fit))

#generate predictions
sqf.data$predicted.probability <- predict(fit, newdata = sqf.data, type='response') 

#Part ii, predict probability for specific person (still working on it)
newdata <- data.frame(age = c(30), suspect.height=c(6), suspect.weight=c(165), suspect.build=c('medium'), location.housing=c('transit'), additional.highcrime='TRUE', stopped.bc.bulge=c('TRUE'), observation.period=c(10), radio.run=c('FALSE'))

# compute AUC on model
fit.pred <- prediction(sqf.data$predicted.probability, sqf.data$found.weapon)
fit.perf <- performance(fit.pred, "auc")
cat('the auc score is', 100*fit.perf@y.values[[1]], "\n") 

#need to somehow filter/break down by year and then plot
