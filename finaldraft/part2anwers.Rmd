...
__The log odds of a stopped person carrying a weapon is increased by 2.59 if they are stopped in transit compared to if they were stopped at home.__
...
testing.set <- testing.set[!is.na(ravi.model.test.predictedps),]
ravi.model.test.predictedps <- ravi.model.test.predictedps[!is.na(ravi.model.test.predictedps)]
...
ravi.calibration.data <- testing.set  %>%
                         ...
                         summarize(model.estimate = mean(ravi.model.test.predictedps),
                                                        numstops = n(),
...
Plotting the performance with both axes on approximately log-scale,
 ...
 ``` {r}
 plot(sort(ravi.model.test.predictedps)[1:1000])
 ```